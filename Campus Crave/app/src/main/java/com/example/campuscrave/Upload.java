...
public class Upload extends AppCompatActivity {
    ...
    EditText uploadName, uploadDesc, uploadAlgn;
    String imageURL;
    Uri uri;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        uploadAlgn = findViewById(R.id.uploadAlgn);
    ...
    public void uploadData(){
        ...
        String algn = uploadAlgn.getText().toString();
        String foodtype;
        if (vegRadioButton.isChecked()) {
            foodtype = vegRadioButton.getText().toString();
        } else if (nonVegRadioButton.isChecked()) {
            foodtype = nonVegRadioButton.getText().toString();
        } else {
            // Handle the case where neither radio button is checked.
            foodtype = "";
        }
        int vote = 0;
        DataClass dataClass = new DataClass(name, desc, algn, imageURL, foodtype,vote);
        //We are changing the child from title to currentDate,
        // because we will be updating title as well and it may affect child value.
        String currentDate = DateFormat.getDateTimeInstance().format(Calendar.getInstance().getTime());
        FirebaseDatabase.getInstance().getReference("Current Food Items").child(currentDate)
